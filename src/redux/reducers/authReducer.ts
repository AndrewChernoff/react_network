...
const getCaptchaImgAC = (captcha:string | null) => ({type: GET_CAPTCHA, captcha}) as const
...
export const logIn = (obj: LoginValues): ThunkAction<void, AppState, unknown, AnyAction> => async(dispatch) => {
    const res = await API.login(obj)
        if (res.resultCode === 0) {    
            dispatch(logInErrorAC(false))
            dispatch(setUserAuthorizedUserThunk())
            dispatch(getCaptchaImgAC(null))
